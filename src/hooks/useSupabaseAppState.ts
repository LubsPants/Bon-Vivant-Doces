...
export function useSupabaseAppState() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('bon-vivant-app-state-sync')
      .on(
        'postgres_changes',
        {
          event: '*',
          ...
          filter: `id=eq.${APP_STATE_ROW_ID}`,
        },
        payload => {
          const nextState = payload.new && typeof payload.new === 'object'
            ? normalizeAppState((payload.new as { state?: unknown }).state)
            : null;
          const nextUpdatedAt = payload.new && typeof payload.new === 'object'
            ? ((payload.new as { updated_at?: string }).updated_at ?? null)
            : null;

          if (!nextState || !nextUpdatedAt) {
            return;
          }
          ...
          writeLocalCache(nextState, nextUpdatedAt);
